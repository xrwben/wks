	...
	var _url = serverPath+'/wks/employee/list?token='+localStorage.getItem("token");
	...
	function qDselectInformation(){
		...
		initFlag=true;
		_url = 'http://192.168.1.140:8080/wks/employee/list';
		_data = {"currPage":currPage+1,"currNum":rows,"FIND_RUZHI_STATUS":'1',"WORK_CITY":kqcs,"COMPANY":gs,"ZHIJI":zj,"SERVICE_TYPE":fwlb,"RUZHI_STATUS":ygzt};
		PageCallback(0);
		closeModal();
	}
	...
	function zzConfirmInfo(employee_id){
		var btn = window.confirm("请确认是否修改员工状态！");
		if(btn == true){
			$.ajax({
				type:"post",
